...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(
        ...
        attn_conditions: Optional[th.Tensor] = None,
        ...
        for layer in self:
            ...
            if isinstance(module, TimestepBlock) and not isinstance(
                ...
                if attn_conditions is not None:
                    attn_cond = attn_conditions.pop(0)
                else:
                    attn_cond = None
                x = layer(
                    x,
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            ...
            if temporal_conv is None else \
            temporal_conv(channels, self.out_channels, kernel_size, padding=padding)
        )
        ...
        self.out_layers = nn.Sequential(
            ...
            zero_module(
                ...
                if temporal_conv is None else \
                temporal_conv(self.out_channels, self.out_channels, kernel_size, padding=padding)
            ),
        )
